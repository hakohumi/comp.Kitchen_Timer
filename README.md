...
- 確認したいこと
  - カウントダウンをするタイマの周期がずれている
    - タイマを再スタートさせるまでに時間が掛かっている

- [ ] タイマ割込みが正確に指定した周期で行われているか
  - 確認方法
    - タイマがスタートして、割込みを行うまでの命令クロック数を把握する
...
CountClassのResetの処理を変更
あとで、フローチャートを変更する必要あり
...
- [x] タイミングチャート
- [x] 実装
  - [x] pin_manager.h にSW割込み許可・禁止のマクロ作成
  - [x] pin_manager.c の各SWの割込みルーチンをフローチャート通りに実装
- [ ] 仕様書 修正
  - [ ] 構成図/ピン配置 修正
...
- 作業予定
  - [x] 設計レビュー 発表資料 まとめる
    - [x] エクセルにフローチャートを貼り付け
  - [x] 仕様変更 カウントダウン一時停止 からカウント時間設定へ遷移するようにする
    - [x] 仕様書 変更
    - [x] 状態遷移図 変更
    - [x] トレーサビリティマトリックス 変更
    - [x] 状態遷移マトリックス 変更
